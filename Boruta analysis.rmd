...
df <- scaled_cross_sections[[2040]]
mRMR_features <- mRMR(x=df[, boruta_features], y=df$scenario_indx)
mRMR_features
```
...
```{r}
tune_rf <- function(data, X_colnames, y_colname) {
  ...
  rf_train <- train(x = data[, X_colnames],
                    y = data[[y_colname]], 
                    trControl = rf_control,
                    tuneGrid = rf_grid,
                    method = "rf")
  
  rf_model <- rf_train$bestTune
